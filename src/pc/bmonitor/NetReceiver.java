...
public class NetReceiver {
	...
	NetReceiver(OutputStream os, InputStream is){
		oStream = os;
		iStream = is;
	...
	private void sendKeyword(String s) throws IOException{
		if(s.equals("BEGIN")){
			...
			это нужно заменить стандартной функцией*/
			int indexIterator = 0;
	...
	private NetReceiver print(String s) throws IOException{
	...
	public void sendDirQuery(String[] dirs) throws IOException, InterruptedException{
		...
		for(int i=0; i < dirs.length ; i++){
			if(i == dirs.length -1){
			//в последнюю строку SPLITTER не дописывается, т.к. в питоне разбиение str.split() добавит лишний хвост ""
				print(dirs[i]);
				break;
			}
			print(dirs[i])
